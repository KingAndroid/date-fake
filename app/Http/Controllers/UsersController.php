...
class UsersController extends Controller {
    ...
    public function login_google(Request $request){
        ...
        $response = json_decode(file_get_contents("https://www.googleapis.com/oauth2/v3/tokeninfo?id_token={$token}"), true);
        if(!isset($response['email'])) return ['success'=>false];
    ...
    public function signup(Request $request){
        ...
        if(empty($user_details['location']) && !empty($user_details['zipcode'])){
	...
	public function registerStore(Request $request){
		...
		if ($validator->fails()) {
			...
			$user->profileType = $request->input('profileType');
			$user->photo = $request->input('profile_image');
            $user->ip_address = $request->ip();
			$user->save();			
			DB::table('role_user')->insert(array('user_id'=>$user->id,'role_id'=>3));			
